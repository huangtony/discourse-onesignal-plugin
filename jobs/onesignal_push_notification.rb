  ...
  class OnesignalPushNotification < Jobs::Base
    ...
    def execute(args)
      ...
      when Notification.types[:mentioned]
        heading = "#{sender} mentioned you - #{topic_title}"
        contents = excerpt
      when Notification.types[:replied]
        heading = "Reply from #{sender} - #{topic_title}"
        contents = excerpt
      when Notification.types[:private_message]
        heading = "Message from #{sender} - #{topic_title}"
        contents = excerpt
      when Notification.types[:posted]
        heading = "#{sender} posted in #{topic_title}"
        contents = excerpt
      when Notification.types[:linked]
        heading = "#{sender} linked to your post - #{topic_title}"
        contents = excerpt
      else
        heading = topic_title
